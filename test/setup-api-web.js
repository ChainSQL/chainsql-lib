...
const {DacAPI, DacAPIBroadcast} = require('ripple-api');
...
function setup(port_ = port) {
  const tapi = new DacAPI({server: baseUrl + port_});
  return tapi.connect().then(() => {
    ...
    return new Promise((resolve, reject) => {
      this.api = new DacAPI({server: baseUrl + got.port});
...
function setupBroadcast() {
  ...
  this.api = new DacAPIBroadcast(servers);
  return new Promise((resolve, reject) => {
    this.api.connect().then(() => {
      this.api.once('ledger', () => resolve());

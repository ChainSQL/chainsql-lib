...
const {validate, iso8601ToDacTime} = utils.common
...
function createOrderTransaction(account: string, order: Order): Object {
  const takerPays = utils.common.toDacdAmount(order.direction === 'buy' ?
    order.quantity : order.totalPrice)
  const takerGets = utils.common.toDacdAmount(order.direction === 'buy' ?
  ...
  if (order.expirationTime !== undefined) {
    txJSON.Expiration = iso8601ToDacTime(order.expirationTime)
  }
